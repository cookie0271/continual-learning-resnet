...
##-------------------------------------------------------------------------------------------------------------------##
ROTATION_SWEEP_PRESET = [0.0, 15.0, 45.0, 30.0, 60.0, 75.0, 90.0, 105.0, 120.0, 135.0, 150.0, 165.0, 180.0]
...
def add_problem_options(parser, pretrain=False, no_boundaries=False, **kwargs):
    ...
    if not pretrain:
        ...
        problem_params.add_argument(
            ...
            default=None,
            help=(
                "Rotate contexts 2..T by the given angle in degrees (CIFAR-10 controlled rotation). "
                "Only supported for Domain-IL scenarios."
            ),
        )
        problem_params.add_argument(
            '--rotation-angle-sweep',
            nargs='*',
            type=float,
            metavar='DEG',
            help=(
                "Run multiple controlled-rotation experiments sequentially (Domain-IL, CIFAR-10 only). "
                "Provide a space-separated list of angles (in degrees); if omitted, the preset sequence "
                f"{', '.join(str(int(v)) for v in ROTATION_SWEEP_PRESET)} is used."
            ),
        )
    if pretrain:
...
def add_cl_options(parser, main=False, compare_all=False, compare_replay=False, compare_hyper=False,
    ...
    if not compare_replay:
        ...
        if main and not no_boundaries:
            ...
            param_reg.add_argument('--si', action='store_true', help="select defaults for 'SI' (Zenke et al, 2017)")
            param_reg.add_argument("--mas", action="store_true", help="use 'MAS' (Aljundi et al, 2018)")
            param_reg.add_argument("--rwalk", action="store_true", help="use 'RWalk' (Chaudhry et al, 2018)")
            ...
            param_reg.add_argument("--importance-weighting", type=str, choices=['fisher', 'si', 'owm', 'mas', 'rwalk'],
                                   help="how to measure parameter importance?")
