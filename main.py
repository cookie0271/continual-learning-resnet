...
def handle_inputs():
    ...
    parser.add_argument('--ablation_fixed_numerator', action='store_true', default=False,
                        help='Use fixed numerator in adaptive regularization (ablation)')
    parser.add_argument('--fixed_numerator_value', type=float, default=10.0,
                        help='Value for fixed numerator in adaptive regularization (default: 10.0)')
    parser.add_argument('--ablation_fixed_denominator', action='store_true', default=False,
                        help='Use fixed denominator in adaptive regularization (ablation)')
    parser.add_argument('--fixed_denominator_value', type=float, default=10.0,
                        help='Value for fixed denominator in adaptive regularization (default: 10.0)')

    # Geometry mismatch attack (GMA)
    parser.add_argument('--gma-attack', action='store_true', default=False,
                        help='Enable the geometry mismatch attack (GMA) loss on later tasks')
    parser.add_argument('--gma-gamma', type=float, default=0.0,
                        help='Strength of the GMA loss term (gamma)')
    parser.add_argument('--gma-tag', type=str, default=None,
                        help='Optional identifier (e.g., method name) stored with the GMA metrics log')
...
def run(args, verbose=False):
    ...
    model.gma_attack = args.gma_attack
    model.gma_gamma = args.gma_gamma
    model.gma_tag = args.gma_tag
    ...
    if checkattr(args, 'results_dict'):
        ...
        # Store auxiliary statistics for the GMA experiment
    if checkattr(args, 'gma_attack'):
        lambda_history_full = getattr(model, 'lambda_t_history', [])
        lambda_history_attack = getattr(model, 'lambda_t_history_since_reference', None)
        lambda_history = lambda_history_attack if lambda_history_attack else lambda_history_full
        lambda_mean = float(np.mean(lambda_history)) if len(lambda_history) > 0 else None
        lambda_mean_full = float(np.mean(lambda_history_full)) if len(lambda_history_full) > 0 else None
        gma_summary = {
            'param_stamp': param_stamp,
            'gamma': args.gma_gamma,
            'gamma_label': f"gamma={args.gma_gamma}",
            'tag': args.gma_tag if args.gma_tag is not None else 'unspecified',
            'task1_acc': accs[0] if len(accs) > 0 else None,
            'task2_acc': accs[1] if len(accs) > 1 else None,
            'average_acc': average_accs,
            'lambda_mean': lambda_mean,
            'lambda_history': lambda_history,
            'lambda_history_full': lambda_history_full,
            'lambda_history_attack': lambda_history_attack,
            'lambda_mean_full': lambda_mean_full,
            'gma_loss_mean': float(np.mean(getattr(model, 'gma_adv_loss_history', [])))
            if len(getattr(model, 'gma_adv_loss_history', [])) > 0 else None,
        }
        gamma_fragment = f"gamma{args.gma_gamma}"
        metrics_path = os.path.join(args.r_dir, f"gma-{gamma_fragment}-{param_stamp}.json")
        with open(metrics_path, 'w') as gma_file:
            json.dump(gma_summary, gma_file, indent=2)
        if verbose:
            print(f"Stored GMA metrics at {metrics_path}")
