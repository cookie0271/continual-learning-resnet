...
def train_cl(model, train_datasets, iters=2000, batch_size=32, baseline='none',
    ...
    for context, train_dataset in enumerate(train_datasets, 1):
        ...
        if context<len(train_datasets) and isinstance(model, ContinualLearner):
            ...
            # Store reference statistics for the geometry mismatch attack after the first task
            if context == 1 and getattr(model, 'gma_attack', False):
                model.store_gma_reference(train_dataset, allowed_classes=allowed_classes)
