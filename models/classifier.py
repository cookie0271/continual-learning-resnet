...
class Classifier(ContinualLearner, MemoryBuffer):
    ...
    def store_gma_reference(self, dataset, allowed_classes=None):
        """Forward the helper to ``ContinualLearner`` when available.

        Older checkpoints or custom subclasses might not yet expose the attack utilities, so
        guarding the `super()` call provides a clearer error message and keeps the call-site in
        ``train_cl`` simple.
        """

        parent_impl = getattr(super(), "store_gma_reference", None)
        if parent_impl is None:
            raise AttributeError("ContinualLearner does not implement store_gma_reference")
        return parent_impl(dataset, allowed_classes=allowed_classes)

    def geometry_mismatch_adv_loss(self):
        """Forward to the mixin and gracefully handle models without the helper."""

        parent_impl = getattr(super(), "geometry_mismatch_adv_loss", None)
        if parent_impl is None:
            return None
        return parent_impl()
    ...
    def train_a_batch(self, x, y, scores=None, x_=None, y_=None, scores_=None, rnt=0.5, active_classes=None, context=1,
        ...
        # Geometry mismatch attack penalty (if enabled)
        if getattr(self, 'gma_attack', False) and context > 1:
            gma_loss = self.geometry_mismatch_adv_loss()
            if gma_loss is not None and getattr(self, 'gma_gamma', 0.0) != 0.0:
                loss_total += self.gma_gamma * gma_loss
        ...
        return {
            ...
            'gma_adv': gma_loss.item() if gma_loss is not None else 0,
